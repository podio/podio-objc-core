...
@class PKCURLSessionTaskDelegate;

/**
 *  An block responsible for transforming the result of a task (response + data) to the resulting
 *  object to be used as the <body> parameter of PKCRequest. Uses of this block include transforming
 ...
 */
typedef id (^PKCHTTPResponseProcessBlock) (NSURLResponse *response, NSData *data, PKCURLSessionTaskDelegate *delegate);
 ...
 *  object acts as the NSURLSessionDelegate needs to maintain a mapping of NSURLSessionTask -> PKCURLSessionTaskDelegate.
 *  In PodioKit, the responsibility of that is handled by the PKCHTTPClient.
 */
@interface PKCURLSessionTaskDelegate : NSObject
 ...
 *
 *  @param request                 The original PKCRequest for the task.
 *
