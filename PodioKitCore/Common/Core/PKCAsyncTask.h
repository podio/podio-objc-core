...
@class PKCAsyncTaskResolver;

typedef void (^PKCAsyncTaskCompleteBlock) (id result, NSError *error);
typedef void (^PKCAsyncTaskSuccessBlock) (id result);
 ...
 *  @return The task itself.
 */
- (instancetype)onSuccess:(PKCAsyncTaskSuccessBlock)successBlock;
...
@interface PKCAsyncTaskResolver : NSObject
