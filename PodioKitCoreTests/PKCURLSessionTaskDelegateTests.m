//
//  PKCURLSessionTaskDelegateTests.m
//  PodioKit
//
...
@interface PKCURLSessionTaskDelegateTests : XCTestCase

@property (nonatomic, strong) dispatch_queue_t processingQueue;

@end

@implementation PKCURLSessionTaskDelegateTests

- (void)setUp {
  [super setUp];
  
  self.processingQueue = dispatch_queue_create("com.podio.podiokit.PKCURLSessionTaskDelegateTests", DISPATCH_QUEUE_CONCURRENT);
}
...
- (void)testCompletionWithSuccess {
  __block PKCResponse *occurredResponse = nil;
  __block NSError *occurredError = nil;
  
  PKCRequest *request = [PKCRequest GETRequestWithPath:@"/some/path" parameters:nil];
  PKCURLSessionTaskDelegate *delegate = [[PKCURLSessionTaskDelegate alloc] initWithRequest:request
                                                                   responseProcessingQueue:self.processingQueue
                                                                             progressBlock:nil
                                                                      responseProcessBlock:nil
                                                                           completionBlock:^(PKCResponse *response, NSError *error) {
...
- (void)testCompletionWithTransformedResponse {
  ...
  PKCURLSessionTaskDelegate *delegate = [[PKCURLSessionTaskDelegate alloc] initWithRequest:request
                                                                   responseProcessingQueue:self.processingQueue
                                                                             progressBlock:nil
                                                                      responseProcessBlock:^id(NSURLResponse *response, NSData *data, PKCURLSessionTaskDelegate *delegate) {
    return @"A new value";
  } completionBlock:^(PKCResponse *response, NSError *error) {
    occurredResponse = response;
    occurredError = error;
...
- (void)testTaskDidFinishDownloadingToURL {
  ...
  PKCRequest *request = [PKCRequest GETRequestWithPath:@"/some/path" parameters:nil];
  request.fileData = [PKCRequestFileData fileDataWithFilePath:toFilePath name:nil fileName:nil];
...
- (void)testTaskDidReceiveData {
  __block PKCResponse *receivedResponse = nil;
